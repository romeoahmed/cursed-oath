...
同时维护 `zh_cn`、`en_us`、`ja_jp`。日文原作名称用于校准含义，三种语言共享能力 ID 与玩法规则。
...
简体授权正文或字幕尚待直接核验。新增来源需记录语种、发行方、版本及卷/话、时间点或页码；发布前复核选词与异名。无法核实的译名可保留审校后的工作版本，并维持“暂”标记。
...
```text
src/main/resources/assets/cursed-oath/lang/
  en_us.json
  zh_cn.json
  ja_jp.json
```
...
用 `Component.translatable` 构造整句文本，参数采用可重排的 `%1$s`、`%2$s`。技能名作为可翻译子组件，按键名读取实际绑定；本地化数字在客户端格式化。服务端传结构化原因或翻译组件，不调用客户端语言服务。[Fabric Text and Translations](https://docs.fabricmc.net/develop/text-and-translations)

## 验证

`LocalizationTest` 检查 JSON 字符串值、重复键、非空文本、三语键集、术式名称和索引占位符。键是否仍在使用需结合注册及动态生成规则判断，不能仅按文本搜索结果删除。

HUD 按字体实际宽度换行。客户端测试覆盖固定分辨率下的三语截图；小窗口、大 UI 比例、罕见汉字、输入法及长译名仍需专项检查。

审校时区分反转术式与术式反转、极之番与最大输出、御厨子与伏魔御厨子。日文用于语义核对，各语种采用自然表达，不要求逐字直译。
