...
internal class StrikeGeometry(
    ...
    fun dismantle(
        ...
        val (side, up) = EffectMesh.basis(direction)
    ...
    fun blackFlash(
        ...
        for (i in -GRID..GRID) {
            ...
            val width = up.scale(if (core) FLASH_CORE_WIDTH else FLASH_HALO_WIDTH)
            val offset = width.scale(-0.5)
            mesh.ribbon(
                arrayOf(offset, end.scale(BEND).add(side).add(offset), end.add(offset)),
                width,
                if (core) BLACK else CRIMSON,
                1 - progress,
            )
        }
    }
    ...
    private companion object {
        ...
        const val BLACK = 0x090308
        const val FLASH_CORE_WIDTH = 0.08
        const val FLASH_HALO_WIDTH = 0.2
    }
}
