...
class CombatGameTest {
    ...
    fun nativePrimaryHitAppliesBlackFlashPowerAndConsumesPreparation(helper: GameTestHelper) {
        val player = helper.caster()
        CombatRuntime.practice(player, true)
        val fighter = CombatRuntime.fighter(player)
        val target = helper.stationaryTarget(EntityTypes.VILLAGER, BlockPos(2, 1, 3))
        val health = 100f
        checkNotNull(target.getAttribute(Attributes.MAX_HEALTH)).baseValue = health.toDouble()
        target.health = health
        val damage = 4.0
        checkNotNull(player.getAttribute(Attributes.ATTACK_DAMAGE)).baseValue = damage
        val chargeTicks = 20
        repeat(chargeTicks) { player.doTick() }
        fighter.preparePulse()
        // Fix only the random roll; native attack, damage events and the mixin still run.
        val rollSeed = 4096L
        player.random.setSeed(rollSeed)
        player.attack(target)
        val expectedDamage = 32f
        helper.assertTrue(target.health == health - expectedDamage, "Native Black Flash applies 4^2.5 damage")
    ...
    fun cleaveUsesFourBlockSurfaceReach(helper: GameTestHelper) {
        ...
        val miss = checkNotNull(helper.reserveTerrain(player))
        ...
        val hit = checkNotNull(helper.reserveTerrain(player))
        try {
    ...
    fun cancellationAndReconnectPreserveResourceObligations(helper: GameTestHelper) {
        ...
        fighter.cancel()
        val reconnected = helper.fighter(player)
