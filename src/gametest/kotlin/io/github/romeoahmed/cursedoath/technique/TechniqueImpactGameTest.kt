...
import io.github.romeoahmed.cursedoath.release
...
class TechniqueImpactGameTest {
    ...
    fun blueDeliversItsCompleteCompressionOutput(helper: GameTestHelper) {
        ...
        helper.release(player, Technique.BLUE)
    ...
    fun removingPurpleDuringDamageStopsRemainingContacts(helper: GameTestHelper) {
        removalDuringDamage(helper, Technique.PURPLE)
    }

    @GameTest(environment = "cursed-oath-test:waves", structure = "cursed-oath-test:arena", maxTicks = 100)
    fun removingDismantleDuringDamageStopsRemainingContacts(helper: GameTestHelper) {
        removalDuringDamage(helper, Technique.DISMANTLE)
    }

    private fun removalDuringDamage(
        helper: GameTestHelper,
        technique: Technique,
    ) {
        val player = helper.caster().apply { setPos(helper.absoluteVec(ORIGIN)) }
        val targets = List(2) { helper.stationaryTarget(EntityTypes.VILLAGER, TARGET) }
        val wave = helper.launchWave(player, technique)
        helper.allowDamage { target, _, _ ->
            if (target in targets) wave.discard()
            true
        }
        helper.succeedWhen {
